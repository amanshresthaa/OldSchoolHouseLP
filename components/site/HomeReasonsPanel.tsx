...
export function HomeReasonsPanel({
  ...
  return (
    <div
      className={cn("surface-pane space-y-4 md:px-6 md:py-6", className)}
      {...props}
    >
      <style>{`
          ...
          >
            <div
              ...
              style={
                isCueAnimating
                  ? {
                      ["--osh-reasons-peek-offset" as string]: `${railPeekOffset}px`,
            ...
            >
              {reasons.map((reason, index) => (
                <article
                  key={reason.title}
                  className={cn(
                    "surface-pane relative isolate flex min-h-[17.5rem] snap-start flex-col overflow-hidden rounded-[1.5rem] border border-[rgba(196,189,181,0.45)] bg-[linear-gradient(180deg,rgba(255,255,255,0.94)_0%,rgba(246,241,235,0.98)_100%)] shadow-[0_18px_34px_rgba(31,34,29,0.08)] sm:min-h-0 sm:px-5 sm:py-4 sm:shadow-none",
                    index % 2 === 1 &&
                ...
                >
                  <div className="absolute inset-x-0 top-0 h-20 bg-[radial-gradient(circle_at_top_left,rgba(245,208,107,0.14),transparent_56%)] sm:h-14" />
                  ...
                  <div className="relative flex items-center justify-between gap-3">
                    ...
                    <span className="h-px flex-1 bg-[rgba(175,43,62,0.18)]" />
                  </div>

                  <div className="relative pt-5 sm:pt-4">
                    <h3 className="max-w-[15rem] font-heading text-[1.72rem] leading-[1.04] tracking-[-0.035em] text-primary sm:max-w-none sm:text-[1.18rem] sm:leading-[1.16]">
                      {reason.title}
                    </h3>
                    <p className="pt-3 text-sm leading-6 text-on-surface sm:pt-2 sm:text-[0.92rem] sm:leading-5">
                      {reason.description}
                    </p>
                  </div>

                  <div className="mt-auto flex items-end justify-between gap-4 pt-6 sm:pt-4">
